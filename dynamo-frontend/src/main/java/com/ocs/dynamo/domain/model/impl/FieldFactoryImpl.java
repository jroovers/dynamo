...
public class FieldFactoryImpl implements FieldFactory {
    ...
    public <U> void addConvertersAndValidators(BindingBuilder<U, ?> builder, AttributeModel am, Converter<String, ?> customConverter) {
        if (am.isEmail()) {
            BindingBuilder<U, String> sBuilder = (BindingBuilder<U, String>) builder;
            sBuilder.withNullRepresentation("")
                    .withValidator(new EmailValidator(messageService.getMessage("ocs.no.valid.email", VaadinUtils.getLocale())));
        } else if (am.isWeek()) {
            BindingBuilder<U, String> sBuilder = (BindingBuilder<U, String>) builder;
            ...
            if (customConverter == null) {
                if (am.getType().equals(BigDecimal.class)) {
                    sBuilder.withConverter(ConverterFactory.createBigDecimalConverter(am.isCurrency(), am.isPercentage(),
                            SystemPropertyUtils.useThousandsGroupingInEditMode(), am.getPrecision(), VaadinUtils.getCurrencySymbol()));
                } else if (NumberUtils.isInteger(am.getType())) {
                    sBuilder.withConverter(ConverterFactory.createIntegerConverter(SystemPropertyUtils.useThousandsGroupingInEditMode(),
                            am.isPercentage()));
                } else if (NumberUtils.isLong(am.getType())) {
                    sBuilder.withConverter(
                            ConverterFactory.createLongConverter(SystemPropertyUtils.useThousandsGroupingInEditMode(), am.isPercentage()));
                } else if (NumberUtils.isDouble(am.getType())) {
                    sBuilder.withConverter(
                            ConverterFactory.createDoubleConverter(am.isCurrency(), SystemPropertyUtils.useThousandsGroupingInEditMode(),
    ...
    private void postProcessField(final AbstractComponent field, final AttributeModel am, boolean search) {
        ...
        if (am.isCurrency() && field instanceof AbstractTextField) {
            ...
            atf.addBlurListener(event -> {
                String value = atf.getValue();
                if (!StringUtils.isEmpty(value) && value.indexOf(VaadinUtils.getCurrencySymbol()) < 0) {
