 ...
 * Note that creating custom fields may be delegated to other factories by
 * implementing those factories with interface FieldFactory and making sure that
 * these can be found by the service locator.
 *
 * @author bas.rutten
 * @param <T>
...
public class ModelBasedFieldFactory<T> extends DefaultFieldGroupFieldFactory
	...
	public <ID extends Serializable, S extends AbstractEntity<ID>> AbstractField<?> constructComboBox(
			EntityModel<?> entityModel, final AttributeModel attributeModel, final Filter filter,
	...
	public <ID extends Serializable, S extends AbstractEntity<ID>> AbstractField<?> constructInternalLinkField(
		...
		entityModel = resolveEntityModel(entityModel, attributeModel, true);
	...
	public <ID extends Serializable, S extends AbstractEntity<ID>> Field<?> constructCollectionSelect(
			...
			final boolean multipleSelect, final boolean search) {
		final EntityModel<?> em = resolveEntityModel(fieldEntityModel, attributeModel, search);
		...
		if (AttributeSelectMode.LOOKUP.equals(mode)) {
			...
			// fancy list select
			final FancyListSelect<ID, S> listSelect = new FancyListSelect<>(service, (EntityModel<S>) em,
	...
	public <ID extends Serializable, S extends AbstractEntity<ID>> EntityLookupField<ID, S> constructLookupField(
			...
			final boolean search, final boolean multiSelect) {
	...
	private <ID extends Serializable, S extends AbstractEntity<ID>, O extends Comparable<O>> SimpleTokenFieldSelect<ID, S, O> constructSimpleTokenField(
			...
			final boolean elementCollection, final Filter fieldFilter) {
		final BaseService<ID, S> service = (BaseService<ID, S>) serviceLocator
				.getServiceForEntity(entityModel.getEntityClass());
	...
	protected Field<?> constructSelectField(final AttributeModel attributeModel, final EntityModel<?> fieldEntityModel,
		...
		final AttributeSelectMode selectMode = search ? attributeModel.getSearchSelectMode()
				: attributeModel.getSelectMode();
